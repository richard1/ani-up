...
function searchTopResult(query, username, password, callback) {
    search(query, username, password, function(err, results) {
        if(err) {
...
function update(id, episode, username, password, callback) {
    buildAnimeValuesXml(id, episode, function(xml, watchStatus) {
        ...
        request.post({
            ...
            else if(body.indexOf("MySQL Error") != -1) {
                callback("Update error: show does not exist", body);
            }
            else {
                callback(null, {
                    'body': body, 
...
function getTotalEpisodes(id, callback) {
    request.get({
        ...
        if(err) {
            ...
            // scrape the page for episode count label
            var firstPass = body.match(/(Episodes:<\/span>\s)(.*)/g);
            if(firstPass) {
                // try to find a number
                var secondPass = firstPass[0].match(/\d+/g);
                if(secondPass) {
                    callback(null, secondPass[0]);
                }
                else {
                    callback("Parse error: episode not found", body);
                }
            }
            else {
                callback("Parse error: episode not found", body);
